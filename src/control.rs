...
pub async fn run(config: sync::Arc<sync::RwLock<Config>>, mut runner: mpsc::Receiver<Box<Job>>, mut spawner: mpsc::Sender<Box<Job>>) {
        ...
        let mut after = vec![];
        {
            ...
            for ajob in wconfig.jobs.iter_mut() {
                if let Schedule::After(path) = &ajob.schedule {
                    if *path == update.path { after.push(Box::new(ajob.clone())); }
                }
